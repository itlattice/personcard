...
use iboxs\exception\TemplateNotFoundException;
...
class Template
{
    ...
    protected $config = [
        ...
        'ibl_deny_func_list' => 'echo,exit', // 模板引擎禁用函数
        'ibl_deny_php'       => false, // 默认模板引擎是否禁用PHP原生代码
        'ibl_begin'          => '{', // 模板引擎普通标签开始标记
        'ibl_end'            => '}', // 模板引擎普通标签结束标记
        'strip_space'        => false, // 是否去除模板文件里面的html空格与换行
        'ibl_cache'          => true, // 是否开启模板编译缓存,设为false则每次都会重新编译
        ...
        'cache_id'           => '', // 模板缓存ID
        'ibl_replace_string' => [],
        'ibl_var_identify'   => 'array', // .语法变量识别，array|object|'', 为空时自动识别
    ...
    public function __construct(App $app, array $config = [])
    {
        ...
        $this->config['ibl_begin']    = preg_quote($this->config['ibl_begin'], '/');
        $this->config['ibl_end']      = preg_quote($this->config['ibl_end'], '/');

        // 初始化模板编译存储器
        $type = $this->config['compile_type'] ? $this->config['compile_type'] : 'File';

        $this->storage = Loader::factory($type, '\\iboxs\\template\\driver\\', null);
    }
    ...
    {
        if (!$this->config['ibl_cache'] || !is_file($cacheFile) || !$handle = @fopen($cacheFile, "r")) {
            return false;
        }
    ...
    private function compiler(&$content, $cacheFile)
    {
        ...
        // 模板过滤输出
        $replace = $this->config['ibl_replace_string'];
    ...
    private function parsePhp(&$content)
    {
        ...
        // PHP语法检查
        if ($this->config['ibl_deny_php'] && false !== strpos($content, '<?php')) {
        ...
        if (preg_match_all($regex, $content, $matches, PREG_SET_ORDER)) {
            foreach ($matches as $match) {
                ...
                switch ($flag) {
                    ...
                    default:
                        // 未识别的标签直接返回
                        $str = $this->config['ibl_begin'] . $str . $this->config['ibl_end'];
                        break;
                }
                ...
                if (isset($_varParseList[$match[0]])) {
                    ...
                    if (strpos($match[0], '.')) {
                        ...
                        if ('$iboxs' == $first) {
                            ...
                            switch ($this->config['ibl_var_identify']) {
                                case 'array': // 识别为数组
    ...
    public function parseVarFunction(&$varStr, $autoescape = true)
    {
        ...
        if (isset($_varFunctionList[$_key])) {
            ...
            // 取得模板禁止使用函数列表
            $template_deny_funs = explode(',', $this->config['ibl_deny_func_list']);
     ...
     * @return string
     */
    public function parseiboxsVar($vars)
    {
        ...
        if ($vars) {
            ...
            switch ($type) {
                ...
                case 'VERSION':
                    ...
                    break;
                case 'LDELIM':
                    $parseStr = '\'' . ltrim($this->config['ibl_begin'], '\\') . '\'';
                    break;
                case 'RDELIM':
                    $parseStr = '\'' . ltrim($this->config['ibl_end'], '\\') . '\'';
                    break;
                default:
                    if (defined($type)) {
                        $parseStr = $type;
    ...
    private function getRegex($tagName)
    {
        $regex = '';
        if ('tag' == $tagName) {
            $begin = $this->config['ibl_begin'];
            $end   = $this->config['ibl_end'];
