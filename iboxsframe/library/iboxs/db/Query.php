...
use iboxs\Container;
use iboxs\Db;
use iboxs\db\exception\BindParamException;
use iboxs\db\exception\DataNotFoundException;
use iboxs\db\exception\ModelNotFoundException;
use iboxs\Exception;
use iboxs\exception\DbException;
use iboxs\exception\PDOException;
use iboxs\Loader;
use iboxs\Model;
use iboxs\model\Collection as ModelCollection;
use iboxs\model\Relation;
use iboxs\model\relation\OneToOne;
use iboxs\Paginator;
...
class Query
{
    ...
    public function count($field = '*')
    {
        if (!empty($this->options['group'])) {
            ...
            $subSql  = $this->options($options)
                ->field('count(' . $field . ') AS iboxs_count')
                ->bind($this->bind)
                ->buildSql();
    ...
    public function paginate($listRows = null, $simple = false, $config = [])
    {
        ...
        /** @var Paginator $class */
        $class = false !== strpos($config['type'], '\\') ? $config['type'] : '\\iboxs\\paginator\\driver\\' . ucwords($config['type']);
    ...
    public function bind($value, $type = PDO::PARAM_STR, $name = null)
    {
        if (is_array($value)) {
            ...
            $name = $name ?: 'iboxsBind_' . (count($this->bind) + 1) . '_' . mt_rand() . '_';
