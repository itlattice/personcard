...
class TagLib
{
    ...
    {
        $begin   = $this->ibl->config('taglib_begin');
        $end     = $this->ibl->config('taglib_end');
        ...
        if (preg_match_all($regex, $str, $matches)) {
            ...
            if (!empty($this->tags[$name]['expression'])) {
                ...
                if (!isset($_taglibs[$name])) {
                    $_taglibs[$name][0] = strlen($this->ibl->config('taglib_begin_origin') . $name);
                    $_taglibs[$name][1] = strlen($this->ibl->config('taglib_end_origin'));
                }
    ...
    public function parseCondition($condition)
    {
        ...
        $this->ibl->parseVar($condition);

        // $this->ibl->parseVarFunction($condition); // XXX: 此句能解析表达式中用|分隔的函数，但表达式中如果有|、||这样的逻辑运算就产生了歧异
        return $condition;
    }
    ...
    public function autoBuildVar(&$name)
    {
        ...
        $this->ibl->parseVar($name);
        $this->ibl->parseVarFunction($name, false);
