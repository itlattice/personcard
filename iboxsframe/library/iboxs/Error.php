...
use iboxs\console\Output as ConsoleOutput;
use iboxs\exception\ErrorException;
...
class Error
{
    ...
    public static function appError($errno, $errstr, $errfile = '', $errline = 0)
    {
        ...
        if (error_reporting() & $errno) {
            // 将错误信息托管至 iboxs\exception\ErrorException
            throw $exception;
        }
    ...
    public static function getExceptionHandler()
    {
        ...
        if (!$handle) {
            ...
            if ($class && is_string($class) && class_exists($class) && is_subclass_of($class, "\\iboxs\\exception\\Handle")) {
                $handle = new $class;
            } else {
                $handle = new Handle;
