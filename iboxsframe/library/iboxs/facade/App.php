 ...
 * @see \iboxs\App
 * @mixin \iboxs\App
 * @method \iboxs\App bind(string $bind) static 绑定模块或者控制器
 * @method void initialize() static 初始化应用
 * @method void init(string $module='') static 初始化模块
 * @method \iboxs\Response run() static 执行应用
 * @method \iboxs\App dispatch(\iboxs\route\Dispatch $dispatch) static 设置当前请求的调度信息
 * @method void log(mixed $log, string $type = 'info') static 记录调试信息
 * @method mixed config(string $name='') static 获取配置参数
 * @method \iboxs\route\Dispatch routeCheck() static URL路由检测（根据PATH_INFO)
 * @method \iboxs\App routeMust(bool $must = false) static 设置应用的路由检测机制
 * @method \iboxs\Model model(string $name = '', string $layer = 'model', bool $appendSuffix = false, string $common = 'common') static 实例化模型
 * @method object controller(string $name, string $layer = 'controller', bool $appendSuffix = false, string $empty = '') static 实例化控制器
 * @method \iboxs\Validate validate(string $name = '', string $layer = 'validate', bool $appendSuffix = false, string $common = 'common') static 实例化验证器类
 * @method \iboxs\db\Query db(mixed $config = [], mixed $name = false) static 数据库初始化
 ...
 * @method string getiboxsPath() static 获取核心框架目录
 ...
 * @method \iboxs\Container container() static 获取容器实例
 */
