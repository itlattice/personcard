...
// +----------------------------------------------------------------------
// | Copyright (c) 2006-2017 http://iboxsframe.cn All rights reserved.
// +----------------------------------------------------------------------
...
class Config extends Command
{
    ...
    protected function buildCacheContent($module)
    {
        ...
        if (is_file($path . 'tags.php')) {
            ...
            if (is_array($tags)) {
                $content .= PHP_EOL . '\iboxs\facade\Hook::import(' . (var_export($tags, true)) . ');' . PHP_EOL;
            }
        }
        ...
        if ('' == $module) {
            $content .= PHP_EOL . substr(php_strip_whitespace(App::getiboxsPath() . 'helper.php'), 6) . PHP_EOL;

            if (is_file($path . 'middleware.php')) {
                $middleware = include $path . 'middleware.php';
                if (is_array($middleware)) {
                    $content .= PHP_EOL . '\iboxs\Container::get("middleware")->import(' . var_export($middleware, true) . ');' . PHP_EOL;
                }
            }
        }
        ...
        if (is_file($path . 'provider.php')) {
            ...
            if (is_array($provider)) {
                $content .= PHP_EOL . '\iboxs\Container::getInstance()->bindTo(' . var_export($provider, true) . ');' . PHP_EOL;
            }
        }

        $content .= PHP_EOL . '\iboxs\facade\Config::set(' . var_export($config->get(), true) . ');' . PHP_EOL;
