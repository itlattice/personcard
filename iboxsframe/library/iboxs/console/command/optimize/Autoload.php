...
EOF;
        ...
        $namespacesToScan = [
            ...
            'iboxs\\'                   => $app->getiboxsPath() . 'library/iboxs',
            'traits\\'                  => $app->getiboxsPath() . 'library/traits',
    ...
    protected function getPathCode($path)
    {
        ...
        $libPath    = $this->normalizePath(realpath($app->getiboxsPath() . 'library'));
        $extendPath = $this->normalizePath(realpath($app->getRootPath() . 'extend'));
        $path       = $this->normalizePath($path);

        if (strpos($path, $libPath . '/') === 0) {
            $path    = substr($path, strlen($app->getiboxsPath() . 'library'));
            $baseDir = "'" . $libPath . "/'";
